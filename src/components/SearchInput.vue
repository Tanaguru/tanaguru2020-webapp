<template>
	<div class='filter' v-click-outside="closeList">
		<label class='filter__label' for='search-input'>
			...
			<span class='screen-reader-text'> ({{$t('resultAudit.displayOptions.search.filterInfo')}} {{ selection.length }})</span>
		</label>
		...
		<div class='input-container' role='search'>
			<ul class='selected-words' tabIndex='-1' :aria-label="($t('resultAudit.displayOptions.search.selectedTags'))">
				<li class="selected-words__item selected-item" v-for='(tag, i) in selection' :key='i'>
					...
					<button class="btn btn--nude" :id='tag' @click='deleteKeyword(tag)'>
						<icon-base-decorative width="16" height="16" viewBox="0 0 24 24"><icon-close /></icon-base-decorative>
			...
			<div class="select-tags">
				...
				<div class="select-tags__arrow">
					<icon-base-decorative v-if='this.listTagOpen === false' class="angle-down" @click.native='toggleOnClick'><icon-arrow-blue /></icon-base-decorative>
					<icon-base-decorative v-else class="angle-up" @click.native='toggleOnClick'><icon-arrow-blue /></icon-base-decorative>
				</div>
			</div>
		</div>
		...
		<div class="select-list-wrapper">
			...
			<div aria-live='polite' class='screen-reader-text'>
				<p v-if='this.listTagOpen === true'>{{$t('resultAudit.displayOptions.search.tagsList', {length: shownTagList.length})}}</p>
			</div>
