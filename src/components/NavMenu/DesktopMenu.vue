<template>
    <nav role="navigation" class="nav" aria-label="menu-principal">
        <div>
            <router-link :to="'/users/'+ $store.state.user.id" v-if="$store.getters.isLoggedIn" class='link-independent nav__username' :aria-label="'Account settings for '+ currentUser+'\'s profile'">{{ currentUser }}</router-link>
