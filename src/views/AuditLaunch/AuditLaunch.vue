				...
				<div class="launch-header__inner">
					...
					<div class="launch-header__cta">
						<button class="btn btn--default-inverse btn--icon" type="button" @click="startAudit">
							...
							<span v-if="!launchCondition"
								  class="screen-reader-text">{{ $t('form.errorMsg.missingFields') }}</span>
						</button>
							...
							<div class="input-name form-block">
								<div class="checkbox">
									<input
										...
										type="checkbox"
										id="checkbox-basic-auth"
										name="checkbox-basic-auth"
										v-model="auditConfigurationForm.common.activeBasicAuth"
									/>
									<label for="checkbox-basic-auth" class="checkbox__label">{{$t('audit.basicAuth.active')}}</label>
								</div>
							</div>
						</div>
					</div>

					<div v-if="auditConfigurationForm.common.activeBasicAuth">
						<!-- Basic auth -->
						<basic-auth-url-form
							:has-been-sent="hasTryToLaunch"
							:is-valid="isUrlValid"
...
import InputValidationDisplay from "@/views/AuditLaunch/InputValidationDisplay";
...
export default {
	...
	methods: {
		startAudit: function () {
			...
			if(this.auditConfigurationForm.common.activeBasicAuth){
				parameters["BASICAUTH_URL"] = this.auditConfigurationForm.common.url
				parameters["BASICAUTH_LOGIN"] = this.auditConfigurationForm.common.login
				parameters["BASICAUTH_PASSWORD"] = this.auditConfigurationForm.common.password
			}
